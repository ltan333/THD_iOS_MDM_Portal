...
// For each device, it checks if the profile_uuid differs from the assigner profile UUID.
// If different, it reassigns the device to the correct profile via NanoDEP API.
// Finally, it upserts the device into the portal database.
func (s *deviceServiceImpl) HandleDEPDeviceEvent(ctx context.Context, depName string, devices []dto.DEPDevice, assignerProfileUUID string, nanomdmSvc service.NanoMDMService) error {
	if len(devices) == 0 {
		return nil
	}

	// Collect devices that need profile reassignment
	var needsReassign []string
	for _, dev := range devices {
		if dev.SerialNumber == "" {
			continue
		}
		// Check if device needs profile reassignment
		if dev.ProfileUUID != assignerProfileUUID {
			needsReassign = append(needsReassign, dev.SerialNumber)
			tlog.Info("Device profile mismatch, will reassign",
				zap.String("serial", dev.SerialNumber),
				zap.String("current_profile", dev.ProfileUUID),
				zap.String("assigner_profile", assignerProfileUUID))
		}
	}

	// Batch reassign profiles if needed
	if len(needsReassign) > 0 && nanomdmSvc != nil {
		results, err := nanomdmSvc.AssignDEPProfile(ctx, depName, assignerProfileUUID, needsReassign)
		if err != nil {
			tlog.Error("Failed to reassign DEP profiles", zap.Error(err))
		} else {
			for serial, status := range results {
				switch status {
				case "SUCCESS":
					tlog.Info("Successfully reassigned DEP profile", zap.String("serial", serial))
				case "NOT_ACCESSIBLE":
					tlog.Warn("Device not accessible for profile reassignment",
						zap.String("serial", serial),
						zap.String("status", status))
				default:
					tlog.Error("Failed to reassign DEP profile",
						zap.String("serial", serial),
						zap.String("status", status))
				}
			}
	...
	// Upsert all devices to portal DB
	var devMaps []map[string]any
	for _, dev := range devices {
		if dev.SerialNumber == "" {
			continue
		}
		devMaps = append(devMaps, map[string]any{
			...
			"os":            dev.OS,
			"device_family": dev.DeviceFamily,
		})
	}

	if len(devMaps) > 0 {
		if err := s.repo.UpsertFromDEP(ctx, devMaps); err != nil {
			tlog.Error("Failed to upsert DEP devices", zap.Error(err))
			return err
		}
		tlog.Info("Upserted DEP devices", zap.Int("count", len(devMaps)))
	}
