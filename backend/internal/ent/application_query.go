...
func (_q *ApplicationQuery) QueryVersions() *AppVersionQuery {
...
func (_q *ApplicationQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (_q *ApplicationQuery) FirstIDX(ctx context.Context) uint {
...
func (_q *ApplicationQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (_q *ApplicationQuery) OnlyIDX(ctx context.Context) uint {
...
func (_q *ApplicationQuery) IDs(ctx context.Context) (ids []uint, err error) {
...
func (_q *ApplicationQuery) IDsX(ctx context.Context) []uint {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *ApplicationQuery) WithVersions(opts ...func(*AppVersionQuery)) *ApplicationQuery {
	query := (&AppVersionClient{config: _q.config}).Query()
...
func (_q *ApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Application, error) {
	...
	if query := _q.withVersions; query != nil {
		if err := _q.loadVersions(ctx, query, nodes,
			func(n *Application) { n.Edges.Versions = []*AppVersion{} },
			func(n *Application, e *AppVersion) { n.Edges.Versions = append(n.Edges.Versions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ApplicationQuery) loadVersions(ctx context.Context, query *AppVersionQuery, nodes []*Application, init func(*Application), assign func(*Application, *AppVersion)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint]*Application)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(appversion.FieldApplicationID)
	}
	query.Where(predicate.AppVersion(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(application.VersionsColumn), fks...))
	}))
...
func (_q *ApplicationQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(application.Table, application.Columns, sqlgraph.NewFieldSpec(application.FieldID, field.TypeUint))
	_spec.From = _q.sql
